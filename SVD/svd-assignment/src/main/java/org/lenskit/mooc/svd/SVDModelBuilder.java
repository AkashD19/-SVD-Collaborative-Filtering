...
public class SVDModelBuilder implements Provider<SVDModel> {
    ...
    public SVDModel get() {
        ...
        if (featureCount > 0) {
            ...
            logger.info("UserIndex size {}", userIndex.size());
            logger.info("ItemIndex size {}", itemIndex.size());
    ...
    private RealMatrix createRatingMatrix(KeyIndex userIndex, KeyIndex itemIndex) {
        ...
        try (ObjectStream<Rating> ratings = dao.query(Rating.class)
            ...
            logger.debug("Populating matrix");
            for(Rating r : ratings){
                logger.debug("Rating :::");
                long user=r.getUserId();
                long item=r.getItemId();
                double b=baseline.getIntercept()+baseline.getUserBias(user)+baseline.getItemBias(item);
                double rValue=r.getValue()-b;
                logger.debug("User:{} Item:{} Rating:{}", user , item , rValue);

                matrix.setEntry(userIndex.getIndex(user),itemIndex.getIndex(item),rValue);
            }
